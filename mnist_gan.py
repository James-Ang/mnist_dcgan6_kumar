...
def execution(disc_optimize, discriminator_loss, gen_optimize, generator_loss, disc_loss_total, gen_loss_total, X_input, Z_input):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            Z_noise = sess.run(tf.random_uniform([batch_size, z_noise_dim],minval=-1.,maxval=1.))
...
def main():
    ...
    if train:
        ...
        weights, bias = bias_weights()
        
        X_input, Z_input = input_place_holder(z_noise_dim, image_dim)
        
        real_logit, real_disc, fake_logit, fake_disc = building_network(X_input, Z_input, weights, bias)
        
        discriminator_loss, generator_loss = loss(real_logit, real_disc, fake_logit, fake_disc)
        
        disc_loss_total, gen_loss_total = summary(discriminator_loss, generator_loss)
        
        disc_var, gen_var = variables(weights, bias)
        
        disc_optimize, gen_optimize = optimiser(lr, discriminator_loss, disc_var, generator_loss, gen_var)
        
        execution(disc_optimize, discriminator_loss, gen_optimize, generator_loss, disc_loss_total, gen_loss_total, X_input, Z_input)
